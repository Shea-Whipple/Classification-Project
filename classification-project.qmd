...
# Do the same on new data-set

"Kaggle" is a website that runs a variety of data science competitions. Read about the "framingham" heart study and the associated data set [here](https://www.kaggle.com/datasets/dileep070/heart-disease-prediction-using-logistic-regression).
...
framingham$TenYearCHD <- factor(framingham$TenYearCHD, levels = c(0, 1), labels = c("No", "Yes"))

framingham_split <- initial_split(framingham, prop = 0.60)
framingham_train <- training(framingham_split)
framingham_test <- testing(framingham_split)



logregfit_framingham <- logistic_reg() |>
  set_engine("glm") |>
  fit(TenYearCHD ~ ., data = framingham_train)

tidy(logregfit_framingham) |> kable(digits = 3)

test_probabilities_framingham <- predict(logregfit_framingham, new_data = framingham_test, type = "prob")

test_data_with_probs_framingham <- bind_cols(framingham_test, test_probabilities_framingham)
roc_data_framingham <- roc_curve(test_data_with_probs_framingham, truth = TenYearCHD, .pred_Yes, event_level = "second")
auc_value_framingham <- roc_auc(test_data_with_probs_framingham, truth = TenYearCHD, .pred_Yes, event_level = "second")

ggplot(roc_data_framingham, aes(x = 1 - specificity, y = sensitivity)) +
  geom_line() +
  geom_abline(linetype = "dashed") +
  labs(x = "False Positive Rate (1 - Specificity)", y = "True Positive Rate (Sensitivity)", title = "ROC Curve") +
  annotate("text", x = 0.75, y = 0.25, label = paste("AUC =", round(auc_value_framingham$.estimate, 3)), size = 5, color = "blue") +
  theme_minimal()
```
